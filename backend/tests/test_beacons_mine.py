...
def test_list_beacons(client: TestClient, api_base_url: str):
    ...
    with patch(
        "app.services.salt_api.salt_client.list_beacons", new_callable=AsyncMock
    ) as mock_list:
...
def test_add_beacon(client: TestClient, api_base_url: str):
    ...
    with patch(
        "app.services.salt_api.salt_client.add_beacon", new_callable=AsyncMock
    ) as mock_add:
...
def test_delete_beacon(client: TestClient, api_base_url: str):
    ...
    with patch(
        "app.services.salt_api.salt_client.delete_beacon", new_callable=AsyncMock
    ) as mock_delete:
...
def test_get_mine_data(client: TestClient, api_base_url: str):
    ...
    with patch(
        "app.services.salt_api.salt_client.get_mine_data", new_callable=AsyncMock
    ) as mock_get:
...
def test_send_mine_data(client: TestClient, api_base_url: str):
    ...
    with patch(
        "app.services.salt_api.salt_client.send_mine_data", new_callable=AsyncMock
    ) as mock_send:
...
def test_list_returners(client: TestClient, api_base_url: str):
    ...
    with patch(
        "app.services.salt_api.salt_client.list_returners", new_callable=AsyncMock
    ) as mock_list:
        mock_list.return_value = mock_response
